...
tf.flags.DEFINE_string('Y', '/home/root123/data/datasets/target/smallmalaria/',
                       'Y tfrecords file for training, default: data/tfrecords/orange.tfrecords')
tf.flags.DEFINE_string('load_model', 'LM_crescet/max',
...
x_img_path = '/home/root123/data/malaria/source'
#x_name = x_img_path.split('/')[-1].split('.')[0]
y_img_path = '/home/root123/data/malaria/target'
...
def train():
    ...
    with graph.as_default():
        ...
        G_loss, D_Y_loss, F_loss, D_X_loss, fake_y, fake_x, Disperse_loss, Fuzzy_loss, feature_x, feature_y, re_X, re_Y = cycle_gan.model()
    ...
    with tf.Session(graph=graph) as sess:
        ...
        try:
            ...
            #Cross Domain Image Generation#
                x_img_dir=os.path.join(x_img_path,str(i+1)+'.jpg')
                x_img,_,x_oimg = get_single_img(x_img_dir)
                y_path=os.path.join(y_img_path,str(i+1)+'.png')
                y_img,_,y_oimg=get_single_img(y_path)
                fake_y_eval,fake_x_eval, conv_y_eval,re_X_eval,re_Y_eval = sess.run(
                [fake_y, fake_x,  tf.get_collection('conv_output'),re_X,re_Y],
                ...
                plot_fake_xy(fake_y_eval[0], fake_x_eval[0], str(i+1), str(i+1), x_oimg,y_oimg,fake_dir,re_X_eval[0],re_Y_eval[0])
                print('processing:',i)
            ...
            #Feature Map Visualization#
                ...
                id_y_dir=os.path.join(conv_dir, str(i+1))
                utils.prepare_dir(id_y_dir)
                ...
                cv2.imwrite(os.path.join(id_y_dir, str(i+1)+'.png'), y_oimg)
            ...
            '''
            if True:
                t_img_path = '/home/root123/data/FCGAN/data/1.jpg'
                t_dir = '/home/root123/data/FCGAN/1_toxo'
                t_imgs, t_lbs, t_img = get_single_img(t_img_path)
                feature_y_eval = sess.run(
                    feature_y,
                    feed_dict={cycle_gan.y: t_imgs})
                idx_u = 0
                u_0 = cal_U(feature_y_eval[0], Cy, 2, 2)[idx_u]
                print(u_0)

                for j in range(16):
                    t_img_path = os.path.join(t_dir, str(j + 1) + '.png')
                    t_imgs, t_lbs, t_img = get_single_img(t_img_path)
                    feature_y_eval = sess.run(
                        feature_y,
                        feed_dict={cycle_gan.y: t_imgs})
